...
public class TrailVisual extends GameObject {
	CircleGameObject target;
	float r,g,b,a,size;
	int duration;
	boolean shrink,fade;
	private List<TrailSection> trail = new LinkedList<TrailSection>();
	public TrailVisual(CircleGameObject target,float r,float g,float b,float a, float size,int duration,boolean shrink,boolean fade){
		this.target = target;
		this.r = r;
		this.g = g;
		this.b = b;
	...
	public TrailVisual(CircleGameObject target,float r,float g,float b,float a,float size, int duration){
	...
	public void update() {
		if(target.remove){
			...
			trail.add(new TrailSection(target.px, target.py, duration));
		}
	...
	public void render() {		
		...
		while(iter.hasNext()){
			...
			if(trail.size()==1){
				...
				glTexCoord2f(0,1);glVertex2f((-nx+ny)/ndist+t.x,(-ny-nx)/ndist+t.y);
				glTexCoord2f(0.5f,0);glVertex2f((-ny)/ndist+t.x,(-nx)/ndist+t.y);
				glTexCoord2f(0.5f,1);glVertex2f((+ny)/ndist+t.x,(+nx)/ndist+t.y);
				...
				glTexCoord2f(0,1);glVertex2f((-px+py)/pdist+t.x,(-py-px)/pdist+t.y);
				glTexCoord2f(0,0);glVertex2f((-px-py)/pdist+t.x,(-py+px)/pdist+t.y);
			}else{
				TrailSection tn = iter.next();
				iter.previous();
				TrailSection tp = iter.previous();
